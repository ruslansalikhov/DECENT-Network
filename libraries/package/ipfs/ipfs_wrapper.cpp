...
namespace ipfs {
    ...
    bool IpfsWrapper::ipfs_file_add(const std::string &filename, const std::string &ipfs_url, std::string &cid) {
        ...
        ret_val = go_ipfs_cache_file_add_wrapped(const_cast<char *>(filename.c_str()),
                                                 const_cast<char *>(ipfs_url.c_str()));
        if (!(bool) ret_val.r0) {
            return false;
        }

        cid = std::string(ret_val.r1);
        free(ret_val.r1);
        return true;
    }
    ...
    bool IpfsWrapper::ipfs_bitswap_ledger(const std::string &peer_id, std::string &json_info) {
        ...
        json_info = std::string(ledger.r1);
        free(ledger.r1);
        return true;
    }
